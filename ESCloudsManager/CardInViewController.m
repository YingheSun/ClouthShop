...
@interface CardInViewController ()<AVCaptureMetadataOutputObjectsDelegate>{
    NSTimer *_timer;
    BOOL  _flag;
    BOOL  _isOn;
...
@implementation CardInViewController
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    if (metadataObjects.count > 0) {
        ...
        NSString *compid = kgetLocalData(@"compid");
        NSString *checkStr = [NSString stringWithFormat:@"http://www.esclouds.cn/?compid=%@",compid];
        if([metadataObject.stringValue isEqualToString:checkStr]){
             [self requestForCardIn:metadataObject.stringValue];
        }else{
             kPError(@"打卡失败,打卡二维码错误");
        }
...
-(void)requestForCardIn:(NSString *)shopId{
    ...
    [XSHttpTool GET:@"ESUserCard.UserCard" param:dic success:^(id responseObject) {
        NSDictionary *jsonDic = [[NSDictionary alloc]init];
        jsonDic = responseObject;
        NSLog(@"打卡返回json：%@",jsonDic);
        if ([jsonDic[@"ret"] intValue] == 200) {
            NSLog(@"打卡成功");
//            NSDictionary *timeDic = [jsonDic objectForKey:@"data"];
//            NSLog(@"%@",timeDic);
//            NSInteger timeSp = [[timeDic objectForKey:@"time"] intValue];
//            CardInModel *TimeModel = [CardInModel objectWithKeyValues:responseObject[@"data"][0]];
//            int timeSp = [TimeModel.time intValue];
//            NSDateFormatter* formatter = [[NSDateFormatter alloc] init];
//            [formatter setDateStyle:NSDateFormatterMediumStyle];
//            [formatter setTimeStyle:NSDateFormatterShortStyle];
//            [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
//            NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970: 8 * 3600 + timeSp];
//            NSLog(@"time = %@%d%@",TimeModel,timeSp,confromTimesp);
//            NSString *confromTimespStr = [formatter stringFromDate:confromTimesp];
//            NSLog(@"confromTimespStr =  %@",confromTimespStr);
//            NSString *successMsg = [NSString stringWithFormat:@"打卡成功,时间:%@",confromTimespStr];
            kPSuccess(@"打卡成功");
        }else{
            kPError(jsonDic[@"msg"]);
        }
